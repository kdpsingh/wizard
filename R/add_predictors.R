#' Old add predictors function using for() loops
#' @export
wiz_add_predictors_for_loop = function(wiz_frame = NULL,
                                  variable = NULL,
                                  category = NULL,
                                  lookback = hours(48),
                                  window = lookback,
  ...
  if (!is.null(variable)) {
    message(paste0('Processing variable ', variable, '...'))
  } else if (!is.null(category)) {
  ...
  output_frame = dplyr::bind_rows(output_list) %>%
    tidyr::gather(variable, value, -!!rlang::parse_expr(wiz_frame$temporal_id), -time, -window_time) %>%
...
wiz_calc_stats = function(groups, temporal_id, temporal_variable, temporal_value, stats,
  ...
  suppressWarnings({
    ...
    output_item =
      ...
      dplyr::summarize_at(
        dplyr::vars(dplyr::all_of(temporal_value)),
        .funs = stats) %>%

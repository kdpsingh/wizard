...
#' New furrr-enabled add_predictors function
#' Internal only
wiz_add_predictors_internal_streaming = function(wiz_frame = NULL,
                                                 variables = NULL,
                                                 category = NULL,
                                                 lookback = hours(48),
                                                 window = lookback,
  ...
  suppressWarnings({
    furrr::future_map2(output_frame[[1]],
                       output_frame[[2]],
                       .f = function(id = .x,
                                     time = .y,
                                     temporal_id = wiz_frame$temporal_id,
        ...
        groups =  dplyr::tibble(!!rlang::parse_expr(temporal_id) := id, time = time)
        ...
        output_file_name = if (!is.null(category)) {
          file.path(wiz_frame$output_folder,
                    paste0(filename_prefix, 'temporal', file_type, '_category_', category, '_pid_', Sys.getpid()) %>%
                      janitor::make_clean_names() %>%
                      paste0('.csv'))
        } else {
          file.path(wiz_frame$output_folder,
        ...
        data.table::fwrite(output_item, output_file_name, append = TRUE)
        ...
        # rm(list=ls())
        # gc() # takes time and doesn't do anything
  ...
  # if (output_file == TRUE) {
  #   data.table::fwrite(output_frame, output_file_name, append = TRUE)
  #   message(paste0('The output file was written to: ', output_file_name))
  #   if (log_file) {
  #     write(paste0(Sys.time(), ': The output file was written to: ', output_file_name),
  #           file.path(wiz_frame$output_folder, 'wiz_log.txt'), append = TRUE)
